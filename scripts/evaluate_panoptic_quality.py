...
def main(prediction, ground_truth, output_file=None):

    if not prediction.exists():
        raise Exception(f"Prediction file {str(prediction)} not found.")
    if not ground_truth.exists():
        raise Exception(f"Ground truth file {str(ground_truth)} not found.")

    if output_file is None:
        raise Exception(f"Output file not specified.")
    elif not output_file.parent.is_dir():
    ...
    for idx, gt_inst in enumerate(gt_segs.keys()):
        ...
        if gt_inst not in matches.keys(): # false negative
            ...
            if gt_label not in class_fn_counts.keys():
                class_fn_counts[gt_label] = 1
            else:
                class_fn_counts[gt_label] += 1
        ...
        if gt_label == pred_label: # true positive
            if gt_label not in class_tp_counts.keys():
                class_tp_counts[gt_label] = 1
            else:
                class_tp_counts[gt_label] += 1

            if gt_label not in class_tp_iou_n.keys():
                class_tp_iou_n[gt_label] = 1
    ...
    for idx, pred_inst in enumerate(pred_segs.keys()):
        ...
        if pred_inst not in matched_pred_inst: # false positive
            ...
            if pred_label not in class_fp_counts.keys():
                class_fp_counts[pred_label] = 1
            else:
                class_fp_counts[pred_label] += 1
    ...
    for i, l in enumerate(EVAL_LABELS):
        ...
        if l not in class_tp_iou_n.keys():
            SQ[l] = 0
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-p", "--prediction", type=Path, dest="prediction",
                        help="Path to predicted labels of ground truth mesh vertices (.txt)")

    parser.add_argument("-gt", "--ground_truth", type=Path, dest="ground_truth",
                        help="Path to gt labels of ground truth mesh vertices (*.txt)")
